    ...
    return `
      <article class="card reveal">
        <div class="card-media">
          <img src="${p.image}" alt="${p.name}" loading="lazy" width="720" height="540" />
        </div>
        <div class="card-body">
  ...
  if (slidesEl && dotsEl) {
    ...
    slidesEl.innerHTML = feats.map((p, i) => `
      <div class="slide${i === 0 ? " active" : ""}" role="group" aria-label="${i + 1} of ${feats.length}">
        <div class="slide-media"><img src="${p.image}" alt="${p.name}" ${i === 0 ? "" : 'loading="lazy"'} width="720" height="540" /></div>

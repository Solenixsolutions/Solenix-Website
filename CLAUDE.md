...
- `solenix-website/` — **the editable source site** (deploy this folder to GitHub Pages).
  ...
  - `js/main.js` — renders catalog/search/filters/nav-dropdown/footer-links/featured-slider from that data; wires the enquiry form and scroll UI. Rarely needs editing. Its `cardHTML()` is mirrored by `tools/gen-seo.js` (see below) — change both together. Both emit each product image as a `<picture>` (WebP `<source>` + JPEG `<img>` fallback).
  - `images/products/01.jpg…27.jpg` — one photo per product (720×540 studio shots); other images are hero/cta/factory/logo/favicon. Every JPEG photo also has a same-basename `.webp` (generated by `tools/gen-webp.py`); the site serves WebP — `<picture>` for `<img>`s, CSS `image-set()` for the hero/cta backgrounds — with the JPEG as automatic fallback.
  - `fonts/` — **self-hosted webfonts** (`.woff2`): Saira Semi Condensed 500/600/700/800, Inter (variable 400–700), IBM Plex Mono 500. `@font-face` rules sit at the top of `css/style.css`; nothing loads from Google Fonts (`fonts.googleapis.com`/`fonts.gstatic.com`) any more.
...
- **Featured slider** is driven by the hardcoded `FEATURED` SKU list in `main.js` (`["01","04","08","07","09"]`), not by product data.
...
Colors via CSS vars: `--navy-700 #16324f`, `--amber-500 #f2a414`, `--paper #f5f7f8`, etc. Fonts are self-hosted in `fonts/` (`@font-face` + `font-display:swap`; no Google CDN): Saira Semi Condensed (display), Inter (body), IBM Plex Mono (kickers/SKUs). The display family's real Google Fonts name has a space — "Saira Semi Condensed"; the old CDN link mis-spelled it "SemiCondensed", so headings silently fell back to system sans-serif until self-hosting fixed it (v17). The `--display` CSS var still reads `"Saira SemiCondensed"`, matched by the `@font-face`. Motion (hero kenburns, `.reveal`→`.in-view` IntersectionObserver reveals, hover lifts) is all gated behind `prefers-reduced-motion`. Layout must stay responsive with no horizontal overflow; nav links hide ≤960 px (category chips still filter).

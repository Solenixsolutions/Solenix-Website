...
- `solenix-website/` — **the editable source site** (deploy this folder to GitHub Pages).
  ...
  - `js/main.js` — renders catalog/search/filters/nav-dropdown/footer-links/featured-slider from that data; wires the enquiry form and scroll UI. Rarely needs editing.
  - `images/products/01.jpg…23.jpg` — one photo per product (720×540 studio shots); other images are hero/cta/factory/logo/favicon.
...
- **Add/change a product:** edit `js/products.js` only. Append a block to `PRODUCTS` with `{ sku, name, category, image, desc, tags }`; `category` must exactly match a `CATEGORIES` entry; drop the photo in `images/products/`. The grid, search, category chips, nav dropdown, and footer product links all regenerate from this array. Add a new category by adding its name to `CATEGORIES`.

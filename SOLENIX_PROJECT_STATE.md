# SOLENIX WEBSITE — PROJECT STATE (build v17)

## Project
Static website for Solenix Solutions (solar mounting hardware manufacturer, Jalandhar, Punjab).
Hosting: **live on GitHub Pages at https://solenixsolutions.com** (custom domain via Cloudflare DNS, DNS-only/grey-cloud, HTTPS enforced). No frameworks; vanilla HTML/CSS/JS. Current build: **v17** (footer build-tag shows version). v13 added SEO canonical + Open Graph/Twitter social-preview meta tags (og:image = hero-bg.jpg) so shared links render an image+title card. **v14** added GA4 analytics (property G-E77FC0LCBQ) + contact-event tracking. **v15** added the SEO / AI-visibility foundations; **v16** ran a site-wide image `alt` audit (Part 2.7); **v17** completed Part 2.6 performance — self-hosted the three webfonts (fixing a family-name typo that had kept the Saira display font from ever loading) and shipped WebP versions of every photo with JPEG fallback — see the "SEO & AI visibility" section below.
...
Fonts (self-hosted in `fonts/`, `@font-face` + `font-display:swap`; no Google CDN): Saira Semi Condensed (display 700/800 — real family name has a space; the old "SemiCondensed" CDN typo meant it never loaded until v17) · Inter (body) · IBM Plex Mono (kickers, SKUs).
...
- `solenix-website/` — **editable multi-file source** (this is what deploys to GitHub Pages, with HIGH-quality images):
  index.html · css/style.css · js/products.js · js/main.js · fonts/ (6 self-hosted .woff2) · images/ (products/01–27.jpg + matching .webp, hero-bg.jpg/.webp, cta-bg.jpg/.webp, factory.jpg/.webp, logo-mark.png, favicon.png, solar-banner.jpg[unused], README.md) · tools/ (gen-seo.js, gen-webp.py)
...
- **Perf (Part 2.6, v17):** hero `preload` + `fetchpriority="high"` (now preloads the WebP); WebP with JPEG fallback for every photo — hero/cta via CSS `image-set()`, products + factory via `<picture>` (~2.1 MB → ~1.1 MB of photos, −48%; regenerate with `tools/gen-webp.py`); the three fonts self-hosted in `fonts/`, dropping the render-blocking Google Fonts request. Verify PageSpeed mobile ≥90 on the live URL after deploy.
...
- **Image alt SEO (Part 2.7, v16):** every product image carries a concise descriptive `alt` — new optional `alt` field in `products.js`, rendered by both `cardHTML`s and the featured slider via `p.alt || p.name`, so it can't drift from the catalog. Factory alt refined (now names Jalandhar); header logo stays `alt=""` on purpose (decorative — the brand link already has `aria-label="Solenix Solutions home"` + visible name text); map iframe already carries a descriptive `title`. WebP images + self-hosted fonts shipped in v17 (see the Perf bullet above).
Remaining ([HUMAN] / owner, from the plan): ✅ Google Search Console (verified + sitemap submitted, 2026-07-24) and ✅ Google Business Profile (live; its share link is now in the `sameAs` array, 2026-07-24) are done. Still open: Bing Webmaster + IndexNow, IndiaMART/TradeIndia/Justdial listings with identical NAP, LinkedIn page. As each new social/listing URL goes live, add it to the `sameAs` array in the Organization JSON-LD (index.html, before `</body>`). Part 3 (per-product `/products/<slug>/` pages by growing gen-seo.js into a mini SSG) is the next big [AGENT] step but needs real spec/FAQ data from the owner.
